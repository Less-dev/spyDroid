...
void connectToVNCServer(const std::string& ip, int puerto) {
  
    // Crear un string que combine ip y puerto
    std::ostringstream oss;
    oss << ip << ":" << puerto;
    std::string serverAddress = oss.str();

    // Copiar la dirección del servidor en vncServerName
    strncpy(vncServerName, serverAddress.c_str(), VNCSERVERNAMELEN);
    vncServerName[VNCSERVERNAMELEN - 1] = '\0';

    // config security 
    static UserDialog dlg;  // Make sure the dlg instance exists for the entire connection
    CSecurity::upg = &dlg;
