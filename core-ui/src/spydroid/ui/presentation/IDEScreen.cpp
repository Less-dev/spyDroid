...
IDEScreen::IDEScreen(QWidget *parent) : QWidget(parent), settingsManager(new SettingsManager("ApkStudio", this)) {
    ...
    connect(newFileAction, &QAction::triggered, this, &IDEScreen::newFile);

    openFileAction = new QAction("Abrir Archivo...", this);
    openFileAction->setShortcut(QKeySequence("Ctrl+O"));
    connect(openFileAction, &QAction::triggered, this, &IDEScreen::openFile);

    saveFileAction = new QAction("Guardar", this);
    saveFileAction->setShortcut(QKeySequence("Ctrl+S"));
    connect(saveFileAction, &QAction::triggered, this, &IDEScreen::saveFile);
   ...
   menuBar->setStyleSheet("QMenuBar { background-color: black; }"
    ...
    connect(openFileShortcut, &QShortcut::activated, this, &IDEScreen::openFile);
    connect(fileExplorer, &FileExplorer::fileOpened, this, &IDEScreen::onFileOpened);
    connect(toolBar, &ToolWindowBar::goToHomeIconClicked, this, &IDEScreen::toggleGoToSpydroidStudio);
    connect(toolBar, &ToolWindowBar::fileIconClicked, this, &IDEScreen::toggleFileExplorer);
    connect(toolBar, &ToolWindowBar::gitIconClicked, this, &IDEScreen::toggleGit);
    connect(toolBar, &ToolWindowBar::terminalIconClicked, this, &IDEScreen::toggleTerminal);

    setMinimumSize(600, 500);
}


void IDEScreen::toggleGoToSpydroidStudio() {
    emit goToSpydroidStudio();
}


void IDEScreen::onFileOpened(const QString &filePath) {
    for (int i = 0; i < tabBar->count(); ++i) {
...
void IDEScreen::toggleFileExplorer() {
...
void IDEScreen::toggleGit() {
    // todo
}

void IDEScreen::toggleTerminal() {
...
void IDEScreen::togglePlay() {
    
}


void IDEScreen::newFile() {

}

void IDEScreen::openFile() {
...
void IDEScreen::saveFile() {
    // Save file
}


void IDEScreen::keyPressEvent(QKeyEvent *event) {
...
void IDEScreen::keyReleaseEvent(QKeyEvent *event) {
...
void IDEScreen::paintEvent(QPaintEvent *event) {
